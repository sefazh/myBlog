...
class WebsocketController extends Controller
{
	...
	*/
    ...
    public function init() {
        $this->function['add'] = 'user_add_callback';
        $this->function['send'] = 'send_callback';
        $this->function['close'] = 'close_callback';
        $this->start_server();
    }
        ...
        while(true) {
            ...
            foreach ($this->cycle as $k => $v) {
                ...
                if(!empty($this->function['send'])) {
                    ...
                    $this->close_callback($data, $index, $this);
                }
            }
            sleep(1);
        }
    ...
    public function user_add_callback($ws) {
        $data = count($ws->accept);
        $this->send_to_all($data, 'num', $ws);
    }

    public function close_callback($ws) {
        $data = count($ws->accept);
        $this->send_to_all($data, 'num', $ws);
    }

    public function send_callback($data, $index, $ws) {
